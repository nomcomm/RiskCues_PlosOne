...
Now we specify and estimate (train) a predictive model via multiple regression, and test it using a held out dataset. We read in the test data and apply the trained model to these data: Thus, we multiply the cue-values for the test data with the beta-coefficients from the training data to yield a set of predicted HIV risk values.
```{r}
test_data = read.csv("setB_121-240.txt", header =TRUE, sep = "\t"); 
```


#### 3.1 Linear Regression, reduced set of CONCRETE-only cues
...
rm(test_data, X_test, pred_values, coefficients.reduced, coefficients.full, RegModel.full, RegModel.reduced)
# corrs
```
